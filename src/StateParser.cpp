...
void StateParser::parseTextures(XMLElement* pTextureRoot, std::vector<std::string> *pTextureIDs) {
    for(XMLElement* e = pTextureRoot->FirstChildElement(); e != NULL; e = e->NextSiblingElement()) {
...
void StateParser::parseObjects(XMLElement *pObjectRoot, std::vector<GameObject*> *pObjects) {
    for(XMLElement* e = pObjectRoot->FirstChildElement(); e != NULL; e = e->NextSiblingElement()) {
        ...
        GameObject* pGameObject = GameObjectFactory::Instance()->create(e->Attribute("type")); pGameObject->load(new LoaderParams(x, y, width, height, textureID, numFrames, callbackID, animSpeed));
